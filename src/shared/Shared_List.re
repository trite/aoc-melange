  ...
  L.foldLeft(
    (b, a) =>
      a == delimiter
        ...
        : b
          |> List.initOrEmpty
          |> L.append(b |> L.last |> O.getOrElse([]) |> L.append(a)),
    [[]],
  );
...
let unsafeHead = lst => lst |> L.head |> O.getOrThrow;

let unsafeTail = lst => lst |> L.tail |> O.getOrThrow;
...
let rec transpose =
  ...
  | [[], ..._] => []
  | rows => [
      L.map(unsafeHead, rows),
      ...transpose(L.map(unsafeTail, rows)),
    ];
